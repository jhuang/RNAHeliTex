...
\frametitle{Outline}
   \begin{itemize}
   \item an overview of RNA secondary structure domains and a classical RNA secondary structure prediction algorithm
...
\frametitle{\large Classical secondary structure algorithms (Zuker 1981)}
  ...
  \begin{block}{facts about Zuker algorithm}
    ...
    \item idea: every secondary structure can be attached with an energy value according to some rules,
          the algorithm computes with a given RNA sequence the structure with the minimum free energy.
    \item the runtime is $O(n^3)$
    \item can get only one structure
    \end{itemize}
  \end{block}
\end{frame}
